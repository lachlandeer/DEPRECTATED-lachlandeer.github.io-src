...
In this post I want to show how to estimate a quantile regression model across multiple quantiles and report the results in tabular and graphical formats. 
I've found it hard to find a single reference on how to do this in the past, so I am hoping to fill the void somewhat.
...
First thing we need is to load the necessary packages:
...
We are going to download it directly from the Cameron's website.
...
To do this we can create a vector with the quantiles we are interested in and then estimate the model for each quantile.
...
Each element is the output of a quantile regression, differing by which quantile they are estimated for:
...
The plot should have one subplot for each variable of interest (we'll do all), and each subplot has the point estimate and confidence interval of the regression coefficient for each quantile that we were interested in. 
...
`.x` tells `R` where to use the first argument of map function, which for us is the first argument of the `tidy()` command.
